            ...
            for (int i = 0; i < keys.getFirst().length; i++) {
                ...
                if ((startRow.length == 0 || keys.getSecond()[i].length == 0 ||
                    ...
                    if (splitStart.length == 0) {
                        startRegionSalt = startNullSalt;
                    } else {
                        startRegionSalt = Bytes.toString(splitStart).substring(0, 1);
                    }
                    byte[] splitStartKey = Bytes.toBytes(startRegionSalt + "|" + scanStart);
                    byte[] splitStopKey = Bytes.toBytes(startRegionSalt + "|" + scanStop);
